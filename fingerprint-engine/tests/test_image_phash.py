...
from fingerprint_engine.core.index import InMemoryHashIndex  # noqa: E402 - after sys.path bootstrap
from fingerprint_engine.core.models import Calibration, FingerprintConfig  # noqa: E402 - after sys.path bootstrap
from fingerprint_engine.handlers.image_handler import ImageFileHandler  # noqa: E402 - after sys.path bootstrap
from fingerprint_engine.handlers.image_phash_handler import (  # noqa: E402 - after sys.path bootstrap
    _PHASH_BANDS,
    _PHASH_BITS,
    IMAGE_PHASH_RECOMMENDED_CALIBRATION_KEY,
...
def test_both_image_handlers_are_discovered() -> None:
    # The two image handlers have distinct CLASS names, so discovery keeps both.
    fingerprinter = Fingerprinter()
    names = {h.name for h in fingerprinter.handlers}
    assert {"image", "image_phash"} <= names


# ---------------------------------------------------------------------------
# Mutually-exclusive routing: exactly one image handler claims a file per mode
# ---------------------------------------------------------------------------
...
def test_default_mode_only_raster_claims_phash_declines() -> None:
    # Under the default raster mode the raster handler scores an image and the
    # phash handler declines (score 0.0) -- no routing ambiguity, default path
    # unchanged.
    raster = ImageFileHandler()
    phash = ImagePHashHandler()
    raster.configure(FingerprintConfig())  # default image_mode="raster"
    phash.configure(FingerprintConfig())
    raster_score = raster.can_handle("pic.png", mime_type="image/png", sample=_png_sample())
    phash_score = phash.can_handle("pic.png", mime_type="image/png", sample=_png_sample())
    assert raster_score > 0.0
    assert phash_score == 0.0


def test_phash_mode_only_phash_claims_raster_declines() -> None:
    # Under phash mode the roles flip: the phash handler claims the image with
    # the IDENTICAL score the raster handler would have, and raster declines.
    cfg = FingerprintConfig(image_mode="phash")
    raster = ImageFileHandler()
    phash = ImagePHashHandler()
    raster.configure(cfg)
    phash.configure(cfg)
    raster_score = raster.can_handle("pic.png", mime_type="image/png", sample=_png_sample())
    phash_score = phash.can_handle("pic.png", mime_type="image/png", sample=_png_sample())
    assert raster_score == 0.0
    assert phash_score > 0.0
    # The phash claim equals the score raster gives under its own mode (same
    # underlying image scoring): moving phash to its own handler did not change
    # which files it claims, only the handler name.
    raster.configure(FingerprintConfig())
    assert phash_score == raster.can_handle(
        "pic.png", mime_type="image/png", sample=_png_sample()
    )


def test_phash_mode_fingerprint_routes_to_image_phash_handler(tmp_path: Path) -> None:
    rng = np.random.default_rng(7)
    path = tmp_path / "pic.png"
    _make_image(rng).save(path)
    fingerprint = Fingerprinter(FingerprintConfig(image_mode="phash")).fingerprint_file(path)
    assert fingerprint.handler == "image_phash"


def test_default_mode_fingerprint_routes_to_image_handler(tmp_path: Path) -> None:
    rng = np.random.default_rng(7)
    path = tmp_path / "pic.png"
    _make_image(rng).save(path)
    fingerprint = Fingerprinter().fingerprint_file(path)
    assert fingerprint.handler == "image"
...
def test_phash_handler_honors_per_handler_calibration(tmp_path: Path) -> None:
    # The whole point of the dedicated handler: a stricter cutoff keyed on
    # "image_phash" governs accept/reject for phash fingerprints WITHOUT touching
    # the global default or the raster operating point. A self-match (conf 1.0)
    # is accepted at 0.25; a hypothetical low-confidence hit is rejected, while
    # the same confidence on a different handler is still judged by the default.
    ...
    calibration = Calibration(
        per_handler={
            IMAGE_PHASH_RECOMMENDED_CALIBRATION_KEY: IMAGE_PHASH_RECOMMENDED_MIN_CONFIDENCE
        }
    )
    # The recommended phash cutoff (0.25) is honored for the phash handler...
    assert calibration.min_confidence("image_phash") == IMAGE_PHASH_RECOMMENDED_MIN_CONFIDENCE
    assert calibration.min_confidence("image_phash") == 0.25
    # ...while every other handler keeps the global default (0.05), so promoting
    # phash to its own name does not perturb the raster ("image") operating point.
    assert calibration.min_confidence("image") == calibration.default_min_confidence

    index = InMemoryHashIndex()
    index.add_many([fingerprint])
    result = index.search(fingerprint, top_k=1)[0]
    # A perfect phash self-match clears the strict cutoff; a 0.10 impostor would
    # be rejected at 0.25 but accepted under the lax 0.05 default -- proof the
    # per-handler key is what governs the decision.
    assert calibration.accepts("image_phash", result.confidence)
    assert not calibration.accepts("image_phash", 0.10)
    assert Calibration().accepts("image_phash", 0.10)
